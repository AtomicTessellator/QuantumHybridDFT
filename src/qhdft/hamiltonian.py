...
#   H = T + V_eff
#
#   - T is the kinetic energy (discretized Laplacian)
#
...
def build_hamiltonian(coarseDensity, fineGrid, coarsePoints, shapeFunction, params):
    """Construct sparse Hamiltonian matrix from coarse density.

    Constructs a sparse H (Ng x Ng) from coarse density (NI,).
    ...
    Parameters
    ----------
    coarseDensity : array_like
        Coarse density values of shape (NI,)
    fineGrid : array_like
        Fine grid points of shape (Ng, 1)
    coarsePoints : array_like
        Coarse grid points
    shapeFunction : callable
        Shape function for interpolation
    params : dict
        Parameters containing simulation settings

    Returns
    -------
    H : sparse matrix
        Sparse Hamiltonian matrix of shape (Ng, Ng)
    norm_factor : float
        Normalization factor for eigenvalue scaling
    oracles : dict
        Oracle functions for sparse access in quantum algorithms
    """
